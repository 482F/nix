...
```bash
cd ~
sudo ln -s /mnt/d/wsl/nix-private /nix-private
sudo chown nixos /nix-private
...
cat << EOF > ~/esudo
#!/usr/bin/env bash
sudo NIX_SSL_CERT_FILE="\${NIX_SSL_CERT_FILE}" HTTPS_PROXY="\${HTTPS_PROXY}" NIXPKGS_ALLOW_UNFREE=1 NIXPKGS_ALLOW_INSECURE=1 "\${@}"
EOF
chmod 744 ~/esudo
